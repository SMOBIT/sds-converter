...
def get_image_size_inches(path):
    """
    Gib die Bildgröße in Inches zurück, basierend auf Pixelgröße und DPI.
    """
    img = Image.open(path)
...
def pdf_to_raw_docx(pdf_path, raw_docx_path):
    """
    Konvertiert PDF nach DOCX und extrahiert alle Bilder in einen Nebenordner.
    """
...
def extract_sections(raw_docx_path):
    """
    Liest Abschnitte aus dem Roh-DOCX nach Überschrift 'ABSCHNITT <Nummer>'.
    """
...
def merge_into_template(sections, template_path, out_path):
    """
    Fügt extrahierte Blöcke in das Word-Template an den Platzhaltern ein.
    Erste, raw-Blöcke, dann optional Fallback-Icon.
    """
